...
const Menu = ({ sidebarItems }) => {
  return (
    <ul className="flex md:flex-col gap-x-[20px] md:gap-y-[40px] md:justify-between">
      {sidebarItems.map((item) => (
        <li key={item.name}>
          <NavLink
            to={item.path}
            className={({ isActive }) =>
              `flex items-center w-[45px] h-[45px] rounded-full md:rounded-none md:w-full md:h-max justify-center md:justify-start md:gap-3 md:px-4 md:py-3 md:border-r-[3px] transition ${
                isActive
